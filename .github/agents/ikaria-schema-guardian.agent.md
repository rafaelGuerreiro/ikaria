...
4. Keep shared contract types/constants in `sdks/shared/src/*` and server domain types in each domain `types.rs`; avoid duplicating protocol types in binaries.
5. Confirm server table/type updates stay aligned with shared protocol/domain types.
6. Run root validation before completion (`task check`, `task test`, and root lint workflow).
